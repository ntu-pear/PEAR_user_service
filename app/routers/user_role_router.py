...
def read_user_role(user_role_id: int, db: Session = Depends(get_db)):
    db_user_role = crud_role_user.crud_role_user.get_user_role(db, user_role_id=user_role_id)
...
def read_user_by_roles(role: str, skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    roles = crud_role_user.crud_role_user.get_user_by_roles(db=db, role=role, skip=skip, limit=limit)
    return roles
...
def create_new_user_role(user_role: UserRoleCreate, db: Session = Depends(get_db)):
    return crud_role_user.crud_role_user.create_user_role(db=db, user_role=user_role)
